...
{% if show_first_last %}
    {% if not page.has_previous %}
      <li class="page-item disabled">
        <span class="page-link bg-light" aria-hidden="true" title="{% trans 'First Page' %}">{{ first_label }}</span>
      </li>
    {% else %}
      <li class="page-item">
...
{% for pagenum, index_range, url in page_urls %}
    {% if page.number == pagenum %}
        <li class="page-item active">
          <span class="page-link bg-danger border-danger" aria-label="{% trans 'Current Page' %}" title="{% trans 'Current Page' %}">{% if show_index_range %} {{ index_range }} {% else %} {{ pagenum }} {% endif %}</span>
        </li>
    {% else %}
